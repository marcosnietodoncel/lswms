...
int main(int argc, char** argv)
{
	...
	for( ;; )
	{
		...
		if(!stillImage)
			printf("Fr.#%d - LSWMS: %d lines / %.0f ms , Ang.Error: (Mean, Std)=(%.2f, %.2f)(deg)\n", frameNum, static_cast<int>(lSegs.size()), t, mean.val[0]*180/CV_PI, stddev.val[0]*180/CV_PI);
		else
			printf("LSWMS: %d segments\nAngular Error: Mean = %.2f (deg), Std = %.2f (deg)\nProcess Time = %.0f (ms)\n", static_cast<int>(lSegs.size()), mean.val[0]*180/CV_PI, stddev.val[0]*180/CV_PI,  t);
		if( useWMS )
		...
		if(usePPHT)
		{
			...
			if(!stillImage)
				printf("Fr.#%d - PPHT: %d lines / %.0f ms\n", frameNum, static_cast<int>(lSegsPPHT.size()), t);
			else
				printf("\nPPHT: %d segments\nProcess Time = %.0f (ms)\n", static_cast<int>(lSegsPPHT.size()), t);
			// ++++++++++++++++++++++++++++++++++++++++
		}
